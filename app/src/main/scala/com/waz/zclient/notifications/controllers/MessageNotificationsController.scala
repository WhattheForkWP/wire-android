...
class MessageNotificationsController(bundleEnabled: Boolean = Build.VERSION.SDK_INT > Build.VERSION_CODES.M,
  ...
  //TODO this signal could be lazier
  ...
  private def multipleNotificationProperties(props: NotificationProps, account: UserId, ns: Seq[NotificationData], teamName: Option[Name]): Future[NotificationProps] = {
    ...
    //TODO we only check if n != 1, could be empty!
    val n = ns.head
    ...
    for {
      ...
      if (isSingleConv)
        specProps.copy(
          openConvIntent           = Some((account, convIds.head, requestBase)),
          clearNotificationsIntent = Some((account, Some(convIds.head))),
          action1                  = Some((account, convIds.head, requestBase + 1, bundleEnabled)),
          action2                  = Some((account, convIds.head, requestBase + 2, bundleEnabled))
        )
      else
        specProps.copy(
