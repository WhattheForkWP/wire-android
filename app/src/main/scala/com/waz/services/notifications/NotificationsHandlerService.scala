...
class NotificationsHandlerService extends FutureService {
        ...
        account match {
          case Some(acc) => accs.getZms(acc).flatMap {
            case Some(zms) if ActionClear == intent.getAction =>
              ...
              zms.notifications.dismissNotificationsForConversations(conversation.toSet)
  ...
  object NotificationsHandlerService {
    ...
    def clearNotificationsIntent(userId: UserId, context: Context): PendingIntent =
      PendingIntent.getService(context, userId.str.hashCode, new Intent(context, classOf[NotificationsHandlerService]).setAction(ActionClear).putExtra(ExtraAccountId, userId.str), PendingIntent.FLAG_UPDATE_CURRENT)

    def clearNotificationsIntent(userId: UserId, convId: ConvId, context: Context): PendingIntent =
      PendingIntent.getService(context, userId.str.hashCode + convId.str.hashCode, new Intent(context, classOf[NotificationsHandlerService]).setAction(ActionClear).putExtra(ExtraAccountId, userId.str).putExtra(ExtraConvId, convId.str), PendingIntent.FLAG_UPDATE_CURRENT)

    def checkNotificationsIntent(userId: UserId, context: Context): PendingIntent =
      PendingIntent.getService(context, userId.str.hashCode, new Intent(context, classOf[NotificationsHandlerService]).putExtra(ExtraAccountId, userId.str), PendingIntent.FLAG_ONE_SHOT)

    def quickReplyIntent(userId: UserId, convId: ConvId, context: Context): PendingIntent =
      PendingIntent.getService(context, (userId.str + convId.str).hashCode, new Intent(context, classOf[NotificationsHandlerService]).setAction(ActionQuickReply).putExtra(ExtraAccountId, userId.str).putExtra(ExtraConvId, convId.str), PendingIntent.FLAG_ONE_SHOT)
  }
